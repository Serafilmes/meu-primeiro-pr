...
1 · Check-in (identificar o cartão)         ███████████░  QUASE ✅
        ...
        └─ NOVO (s63): TÚNEL CLOUDFLARE (cloudflared_gma.sh) — QR cai DIRETO na ficha,
           sem a tela de aviso do ngrok grátis + sem o limite de 1 túnel; QR auto-detecta
           a URL (arquivo de estado). ngrok de reserva — validado no set, sai
...
0a. 🚇 **TESTAR o túnel Cloudflare no set (s63)** — o `cloudflared_gma.sh` está pronto e o QR auto-detecta
   a URL, MAS falta: (1) o idealizador rodar **`brew install cloudflared`** (o programa ainda NÃO está
   instalado), (2) **Voltar ao saguão + Entrar** (mudou `flask_gma.py`), (3) rodar `./cloudflared_gma.sh`
   e **escanear o QR no celular** confirmando que cai DIRETO na ficha, sem a tela cinza do ngrok.
   **Se entregar, REMOVER o ngrok** (decisão do idealizador): tirar `ngrok_gma.sh`, o `iniciar_ngrok()` do
   maestro e a leitura do 4040, deixando o Cloudflare como cano único. Depois (opcional): o maestro subir o
   cloudflared sozinho (hoje é à mão) + **modo nomeado** (subdomínio fixo por máquina, resolve a URL que muda).
