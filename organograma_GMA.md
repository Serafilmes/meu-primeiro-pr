...
6 · Inteligência artificial (opcional)      ██░░░░░░░░░░  INICIADA (s45 — 1º tijolo)
        ...
        └─ NOVO (s45): TRANSCRIÇÃO de áudio (Whisper local) CONSTRUÍDA — POR ARQUIVO
           (arquivos.transcricao); caixa isolada .venv_ia + transcritor.py; botão 🎙 na
           planilha (cartão de áudio), status compacto + tela /cartao/<id>/transcricao
        └─ tudo converge na planilha + busca conversacional; nuvem permitida pela qualidade
        └─ próximo: Missão A (busca conversacional lê arquivos.transcricao + classificação)
        └─ pré-requisito da 1ª camada: controle por DATA na aba Listas (grupo por dia)
...
2f. 🔴 **Aviso de login do Google no Painel (DEFERIDO s43)** — a auth do gcloud expira de tempos em
   tempos (política da org) e o exportador falha EM SILÊNCIO → o Sheet para sem aviso (achado no s43:
   parou ~21:50, só percebido pela planilha parada). Desenho: exportador deixa "bilhete de status"
   (ok/login-vencido/sem-internet) → Painel mostra bolinha 🟢/🔴/🟡 + instrução `gcloud auth login`.
   C3+C5. + ação do admin: esticar a sessão Google (admin.google.com → Segurança → Controle de sessão
   do Google Cloud). Ver [[sheets-auth-impersonacao]].
