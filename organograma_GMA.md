...
5 · Tela bonita + várias máquinas           ████░░░░░░░░  EM CONSTRUÇÃO
        ...
        └─ NOVO (s64): MULTI-MÁQUINA (modelo ESTRELA) DESENHADO — 1 servidora central (gma.db+Flask+
           numeração) + estações; storage = NAS EXTERNO (mídia trafega na rede local OK, princípio nº2
           corrigido); dois centros (mídia→NAS / dados→Mac via HTTP); papéis por configuração (não outro
           executável); troca de servidora (só o gma.db viaja) + backup editável. Doc desenho_camada5_…
        └─ a CONSTRUIR (pós-teste): modo ESTAÇÃO (numeração + registro central via HTTP) · backup
           automático do banco · nome mDNS da servidora · .app V1 (embrulho c/ ícone 6floor)
        └─ teste de bancada AMANHÃ (s64): Bloco A/B funcionam hoje (fichas de várias máquinas + satélite),
           Bloco C mede a lacuna; estrela completa = build pós-teste
        └─ falta: conexões por-projeto (Fatia 2) · operadores sincronizados entre máquinas
...
25/06–30/06  S57–S65  🖥️ CAMADA 7 full-dark FECHADO (as 8 telas no mundo escuro 6floor) · REORGANIZAÇÃO DAS TELAS
              (Controle · Mural · Entrega — papéis separados) · túnel Cloudflare = cano único (ngrok removido, testado no set) ·
              multi-máquina ESTRELA desenhado (prep do teste de bancada) · senhas/fix de órfão + ideia de keygen · relatório de cópias
              (detalhe completo no cabeçalho e em contexto_atual_GMA.md)

01/07  S66 📥 CAMADA 1 — CENTRAL DE ENTRADA: importar vocabulário de fontes externas (o "colar link" pendente desde a s31)
           🧱 Novo importador_fontes.py — leitura MECÂNICA (só biblioteca padrão, sem dependência/IA/custo; nunca toca mídia)
           🎛️ Assistente na aba Programação, 3 fontes: Planilha Google · Página de site (AVISA quando é JS → captura comigo) · LINE-UP→programação do dia
           🔗 Despeja em listas_contexto/programacao = o sistema JÁ FECHADO consome sozinho (chips/planilha/IA/cascata palco→show) — zero encanamento novo
           ✂️ Metade A feita (line-up com coluna de dia); Metade B ("pré-criar Posts" / material pedido × entregue) ADIADA pelo idealizador
           ✅ ast + fluxos ponta a ponta em banco temporário (dedup, aviso de data ruim, idempotente); NÃO testado ao vivo (precisa Voltar+Entrar)

01/07  S67 ✍️ CAMADA 3 — ENTREGA aprimorada: FILTROS por coluna (gerados do molde) + LIMPEZA visual (Nome oculta, Caminho no HD em ⋯) +
           PÓS-PRODUÇÃO EDITÁVEL (Editor·Edição·Upload = 1ª tela EDITÁVEL, salva no banco + espelha no Sheets; listas separadas em pessoas_pos;
           operador gere nomes, editor só seleciona) · FATIA 2: login de EDITOR (papel em operadores.py, editor só vê a aba Entrega)

01/07  S68 📄 CAMADA 3 — FATIA 3: RELATÓRIOS DE ENTREGA (fecha o arco da Entrega). Documento final CONGELADO, dois recortes MESMO motor:
           DIÁRIO (seletor de dia — hoje por padrão, reimprime qualquer dia) e FINAL (evento inteiro)
           🧱 Novo relatorios_entrega.py — só LÊ o banco (nunca toca mídia, offline); fonte = a MESMA da tela/Sheets (montar_planilha_com_datas) → nunca diverge
           📊 XLSX (capa + autofiltro + linha congelada) + 📄 PDF (paisagem, marca 6floor) · foto CONGELADA com carimbo de hora (nunca sobrescreve) + download na hora
           🔐 Ação do OPERADOR (editor cai fora pelo portão, não vê a caixa) · dep. nova openpyxl (grátis/offline) · download com travessia de path barrada
           ✅ ast + XLSX/PDF válidos em projeto real + diário filtra certo (4 no dia, 0 no vazio) + portão/download testados; NÃO testado ao vivo (precisa Voltar+Entrar)
```
...
0w. ✅ **Arco da ENTREGA fechado (s67–s68).** A tela Entrega ganhou: **filtros por coluna** + limpeza visual +
   **pós-produção EDITÁVEL** (Editor·Edição·Upload, 1ª tela editável, espelha no Sheets) + **login de editor**
   (papel; editor só vê a Entrega) — s67; e a **FATIA 3 — RELATÓRIOS DE ENTREGA** — s68: documento final
   **CONGELADO** (XLSX + PDF), recorte **diário** (seletor de dia) ou **final**, salvo em `projetos/<slug>/relatorios/`
   + download na hora; ação do operador; novo `relatorios_entrega.py`; dep. nova **openpyxl**. ⏭️ FALTA: **ver ao
   vivo** (Voltar ao saguão + Entrar) e gerar um relatório real. Nota: o molde do **sp2b** está com todas as colunas
   ocultas (teste ao vivo dele) — religar em `/molde` quando usar. Ver [[entrega-pos-producao-editavel]].
0y. ✅ **Central de Entrada — importar vocabulário (s66).** Assistente na aba **Programação** com 3 fontes:
   **Planilha Google** · **Página de site** (avisa quando é JS → captura comigo via Chrome) · **Line-up→programação do dia**
   (palco·show·dia). Leitura mecânica/offline (`importador_fontes.py`), revisão do operador, idempotente; despeja em
   `listas_contexto`/`programacao` que o sistema já fechado consome sozinho (chips/planilha/IA/cascata). ⏭️ FALTA: **ver ao
   vivo** (Voltar ao saguão + Entrar); fatias futuras — **Metade B** (pré-criar Posts / material pedido × entregue, ADIADA),
   fonte **colar lista**, **CSV/PDF/print (OCR)** e o **painel do dia**. Ver [[entrada-listas-vs-importacao]].
0z. ✅ **Telas reorganizadas (s65) — Controle · Mural · Entrega, papéis separados.** Match→**Controle** (ação);
   **Mural** = telão por Post só-leitura (Chegando·Copiando·Copiado, barra de cópia no azulejo); **descartar+nota**
   voltaram pro Controle; **Material recebido** (Copiar agora) atalhado no Controle; **Relatório de cópias** novo
   (tempo+MB/s por cartão, 2 colunas no banco). Regra: status só no Mural, ação só no Controle, velocidade nunca
   na Entrega. ⏭️ FALTA: ver ao vivo (**Voltar ao saguão + Entrar**); cartões antigos mostram "—" no relatório (só
   as próximas cópias gravam tempo/MB/s). Ver [[telas-controle-mural-entrega]].
0a. ✅ **Túnel Cloudflare — FECHADO (s63).** ngrok REMOVIDO; o maestro sobe o cloudflared sozinho com o
   sistema e **TESTADO NO SET** (QR no celular → direto na ficha, sem tela cinza). Sem pendência.
   Fatia futura (opcional, só p/ multi-máquina): **modo nomeado** do Cloudflare — subdomínio fixo por
   máquina (resolve a URL que muda a cada subida; precisa conta+domínio).
